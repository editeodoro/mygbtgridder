...
def read_command_line(argv):
    ...
    parser.add_argument("--beamfwhm", type=float, help="FWHM of the convolution function in arcsec")
    parser.add_argument("-t","--threads", type=int,
                        help="Number of threads to use for gridding")
    parser.add_argument("--nocimpl", default=False, action="store_true",
                        help="Turn off C implementation of gridding algorithm")
...
def gbtgridder(args):
    ...
    if args.beamfwhm is not None:
        beam_fwhm = args.beamfwhm/3600.
    else:
    ...
    if xsize is None or ysize is None:
        ...
        if args.size is not None:
            ...
            #print xRange, yRange, xsky[nonZeroXY].max(), xsky[nonZeroXY].min()
    ...
    if verbose > 4:
        ...
        print("   spectra to grid : ", (wt != 0.0).sum(), "/", len(data[:]))
        print("")
        print("Map info ...")
